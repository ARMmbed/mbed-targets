...
import os
import mimetypes
from mbed_tools_ci.utils.configuration import ConfigurationVariable, configuration
...
def upload_directory(dir: str, bucket_dir: str):
    ...
    for root, dirs, files in os.walk(real_dir_path, followlinks=True, onerror=onerror):
        if root != real_dir_path:
            new_bucket_dir = os.path.join(bucket_dir, os.path.relpath(root, start=dir))
        else:
